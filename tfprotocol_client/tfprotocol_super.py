...
class TfProtocolSuper:
    ...
    def connect(self, keepalive_options: KeepAliveOptions):
        ...
        if final_status.status is StatusServerCode.OK:
            ...
            self._protocol_handler.statusServer(final_status)
            try:
    ...
    def _connect(self) -> StatusInfo:
        self._tcp_timeout_options = TCPTimeoutOptions(
            status_server_callback=self._protocol_handler.statusServer
        )
        count = 0
        ...
        if status.status is not StatusServerCode.OK:
            ...
            self._protocol_handler.responseServerCallback(status)
            return status
        ...
        if status.status is not StatusServerCode.OK:
            self._protocol_handler.statusServer(status)
            return status
        else:
            self._protocol_handler.instanceTfProtocol(self)
